...
test_that("can compute the number of dimensions", {
  expect_equal(rray_dims(x_5), 1)
  expect_equal(rray_dims(x_2x3), 2)
  expect_equal(rray_dims(x_6x1x3), 3)
})

test_that("dimensionality of NULL is 1", {
  expect_equal(rray_dims(NULL), 1)
})

test_that("dimensionality of data frames is 1", {
  expect_equal(rray_dims(mtcars), 1)
})
